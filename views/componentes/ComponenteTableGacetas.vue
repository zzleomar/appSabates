    ...
    >
      <template slot="actions" slot-scope="row">
        <b-button size="sm" @click.stop="row.toggleDetails">
          {{ row.detailsShowing ? 'Ocultar' : 'Mostrar' }} Publicaciones
        </b-button>
      </template>
      <template slot="row-details" slot-scope="row">
        <b-card>
          <ul>
            <li v-for="(value, key) in row.item.decretos" :key="key" style="list-style: none;">
                <div class="col-6">{{value.descripcion}}</div>
            </li>
          </ul>
        </b-card>
      </template>
